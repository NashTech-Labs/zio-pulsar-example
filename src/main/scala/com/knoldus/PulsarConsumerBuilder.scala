...
import org.apache.pulsar.client.api.{ConsumerBuilder, PulsarClientException, Schema, SubscriptionMode, SubscriptionType}
import zio.*
...
class PulsarConsumerBuilder[T: Tag](unsafeConsumerBuilder: ConsumerBuilder[T]) {
  ...
  def withSubType(subType: SubscriptionType) = new PulsarConsumerBuilder(unsafeConsumerBuilder.subscriptionType(subType))
  ...
  def build(schema: Schema[T]) =
    val consumer: IO[PulsarClientException, PulsarConsumer[T]] = ZIO.effect {
